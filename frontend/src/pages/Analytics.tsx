...
import { Card, Row, Col, Statistic, Segmented, Table, Progress, Collapse, Tag, message, Spin, Empty } from 'antd';
import {
  FundOutlined, DollarOutlined, CarOutlined, EnvironmentOutlined,
  BulbOutlined, RiseOutlined, WarningOutlined,
...
const Analytics: React.FC = () => {
  ...
  const zoneColumns = useMemo(() => [
    { title: 'Khu vực', dataIndex: 'zone', key: 'zone' },
    { title: 'Tổng chỗ', dataIndex: 'totalSpots', key: 'totalSpots', width: 100 },
    {
      title: 'Tỷ lệ lấp đầy', dataIndex: 'avgOccupancy', key: 'avgOccupancy', width: 220,
      render: (v: number) => (
        <Progress
          percent={v}
          size="small"
          strokeColor={v >= 80 ? '#ba1a1a' : v >= 60 ? '#934600' : '#1a7a2e'}
        />
      ),
    },
    { title: 'Doanh thu', dataIndex: 'revenue', key: 'revenue', render: (v: number) => `${fmt(v)}đ` },
    { title: 'DT/chỗ', dataIndex: 'revenuePerSpot', key: 'revenuePerSpot', render: (v: number) => `${fmt(v)}đ` },
  ], []);
  ...
  return (
    <div>
      <h2 className="page-title"><FundOutlined /> Phân tích &amp; Gợi ý quyết định</h2>
        ...
        <Spin spinning={loading}>
          <Row gutter={[14, 14]}>
            <Col xs={24} sm={12} xl={5}>
              <Card><Statistic title="Doanh thu" value={data.summary.totalRevenue} formatter={(v) => `${fmt(Number(v))}đ`} prefix={<DollarOutlined style={{ color: '#005daa' }} />} /></Card>
            </Col>
            <Col xs={24} sm={12} xl={5}>
              <Card><Statistic title="Tổng lượt xe" value={data.summary.totalVehicles} prefix={<CarOutlined style={{ color: '#1a7a2e' }} />} /></Card>
            </Col>
            <Col xs={24} sm={12} xl={5}>
              <Card><Statistic title="TB doanh thu/ngày" value={data.summary.avgRevenuePerDay} formatter={(v) => `${fmt(Number(v))}đ`} prefix={<RiseOutlined style={{ color: '#934600' }} />} /></Card>
            </Col>
            <Col xs={24} sm={12} xl={5}>
              <Card><Statistic title="TB lượt xe/ngày" value={data.summary.avgVehiclesPerDay} prefix={<CarOutlined style={{ color: '#6750a4' }} />} /></Card>
            </Col>
            <Col xs={24} sm={12} xl={4}>
              <Card><Statistic title="Tỷ lệ lấp đầy hiện tại" value={data.summary.occupancyRate} suffix="%" prefix={<EnvironmentOutlined style={{ color: '#ba1a1a' }} />} /></Card>
            </Col>
          </Row>
              ...
              <Card title="Lượt xe theo ngày trong tuần">
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={data.dayOfWeekAnalysis}>
                    ...
                    <Bar dataKey="avgVehicles" fill="#005daa" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </Card>
            </Col>
              ...
              <Card title="Lượt xe theo giờ trong ngày">
                <ResponsiveContainer width="100%" height={280}>
                  <LineChart data={data.hourlyAnalysis}>
                    ...
                    <Line type="monotone" dataKey="avgVehicles" stroke="#1a7a2e" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </Card>
            </Col>
